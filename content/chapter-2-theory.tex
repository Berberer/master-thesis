...
In this step of the worklof, the space one candidate has to be selected from, consists of all this valid pipeline, which usually have two or more components.\newline
...
\textcolor{red}{TODO: Explain CASH}

\section{Black Box Optimization}
\label{sec:theory:optimization}
With the formalization of AutoML as a CASH optimization problem the question arises if it can be tackled similar to standard textbook optimization problems, where established optimization algorithms exist.
Unfortunately, the CASH optimization problem is a special case of optimization problem called \textit{Black Box} optimization problem.\newline
...
Finally, with the aid of the \textit{No-Free-Lunch Theorem}, it will be reasoned why these approaches cannot be optimal.

\subsection{Differences to General Optimization Problems}
\label{sec:theory:optimization:differences}
\textcolor{red}{TODO: Explain Black Box problems}\newline
Without the possibility to select a optimal or near optimal candidate analytically, a series of candidates has to selected and evaluated while trying to approximate such a optimal solution.
Now the question remains how candidates from the two presumably large candidate spaces of Model Selection and Model Configuration should be selected for each iteration and how such a approximation can be attempted.\newline
...
\end{itemize}
Each one will be explained individually in the following.
